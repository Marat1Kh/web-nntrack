...
const FlowDiagram: React.FC = () => {
  ...
  const updateLines = () => {
    ...
    if (isCreatingLine) {
      ...
      const fromPos = getPortPosition(isCreatingLine.from.shapeId, isCreatingLine.from.port);
  ...
  const startConnection = (shapeId: string, port: 'left' | 'right', e: React.MouseEvent | React.TouchEvent) => {
    e.stopPropagation();
  ...
  const deleteShape = (shapeId: string, e: React.MouseEvent) => {
    ...
    setLines(prev => prev.filter(line => 
      line.from.shapeId !== shapeId && line.to.shapeId !== shapeId
    ));
          ...
          <svg className="absolute inset-0" width="100%" height="100%">
            ...
            {isCreatingLine && (
              <line
                ...
                strokeWidth={2}
                strokeDasharray="5,5"
                x1={getPortPosition(isCreatingLine.from.shapeId, isCreatingLine.from.port).x}
                y1={getPortPosition(isCreatingLine.from.shapeId, isCreatingLine.from.port).y}
            ...
            {shapes.map((shape) => (
              ...
              >
                ...
                <g className="opacity-0 hover:opacity-100 transition-opacity">
                  ...
                  <text x="75" y="11" fontSize="10" fill="white" textAnchor="middle">✕</text>
                </g>
